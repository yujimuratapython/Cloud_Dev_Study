# 最近はやりのクラウド仮想開発環境を構築しよう。
git,github,Docker,Vscode,クラウドサービス？なんそれって状態から、\
...
◆ローカルマシン環境でのメリット◆
- パソコンごとに環境の選択やライブラリの選択や細々とした設定インストールをする必要がなくなる。
- コンテナ単位で環境を構築するので、問題が生じた環境の破棄及び再生成が容易。
- 先人が作成してくれた環境のパッケージを用いることにより、最小の労力で環境構築が行える。


◆クラウドマシン環境でのメリット◆
- クラウド上にて仮想マシンに環境を構築しているので、RDPで接続するだけで同一環境で作業ができる。
- 環境を常時稼働させることが出来る。（従量課金制なのでお金はかかります。1h10円くらい・・？）
- 耐障害性が高い。
- 仮想マシンの破棄、再生成が容易。
- 仮想マシンなので、何個でも生成できるし、稼働もできる。（従量課金制・・・・以下略
- クラウド上のマシンリソースを利用してるので、ローカル環境には負担がかからない。
...
Azure Portal 仮想マシンページ内　【接続】という項目があり、RDP、SSHの項目を確認するとパブリックIPがわかるので\
